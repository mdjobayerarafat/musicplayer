...
import { FaPlay, FaRandom, FaMusic, FaCompactDisc, FaFire, FaHeart, FaClock } from 'react-icons/fa';
          ...
          <div className="relative z-10 px-4 sm:px-8 pt-16 lg:pt-12 pb-4">
            ...
            {songs.length > 0 && (
              <div className="flex gap-3">
                <button
                  onClick={playAll}
                  className="flex items-center gap-2 px-5 sm:px-6 py-2.5 sm:py-3 bg-rose-600 hover:bg-rose-700 rounded-full font-semibold transition-all transform hover:scale-105 active:scale-95 shadow-lg shadow-rose-600/20 text-sm sm:text-base"
                >
                ...
                <button
                  onClick={playShuffle}
                  className="flex items-center gap-2 px-5 sm:px-6 py-2.5 sm:py-3 bg-white/10 hover:bg-white/20 rounded-full font-semibold transition-all backdrop-blur-sm text-sm sm:text-base"
                >
                  <FaRandom />
                  Shuffle
                </button>
        ...
        <div className="px-4 sm:px-8 py-6 sm:py-8">
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-8 sm:mb-10">
            {[
              ...
              { icon: FaMusic, label: 'All Songs', color: 'from-purple-500 to-violet-500' },
            ].map((cat) => (
              <button
                key={cat.label}
                className="flex items-center gap-3 p-3 sm:p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-all text-left group"
              >
                <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br ${cat.color} flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform`}>
                  <cat.icon className="text-white text-lg sm:text-xl" />
                </div>
          ...
          {albums.length > 0 && (
            <section className="mb-8 sm:mb-10">
              ...
              <div className="flex gap-4 overflow-x-auto pb-4 sm:grid sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 sm:overflow-visible scrollbar-hide">
                {albums.slice(0, 5).map((album) => (
                  <div
                    key={album.$id}
                    className="group bg-white/5 hover:bg-white/10 rounded-xl p-3 sm:p-4 transition-all cursor-pointer min-w-[160px] sm:min-w-0"
                  >
                    <div className="aspect-square rounded-lg bg-rose-600/10 overflow-hidden mb-3 relative">
                      {album.coverImage ? (
                      ...
                      <button className="absolute bottom-2 right-2 w-10 h-10 rounded-full bg-rose-600 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all transform translate-y-2 group-hover:translate-y-0 shadow-lg shadow-rose-600/30">
          ...
          <section>
            ...
            {songs.length === 0 ? (
              <div className="text-center py-16 sm:py-20">
                ...
                <p className="text-gray-500 mb-6 text-sm sm:text-base">Add some music from the admin panel</p>
                <button
