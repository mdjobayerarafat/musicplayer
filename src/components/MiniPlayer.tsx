...
export default function MiniPlayer() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 slide-up">
      {/* Progress bar */}
      <div className="h-[3px] bg-gray-800/80">
        <div className="h-full bg-rose-500 transition-all duration-200 rounded-r-full" style={{ width: `${progress}%` }} />
      </div>

      <div className="bg-[#111]/95 backdrop-blur-xl border-t border-white/[0.06] px-3 sm:px-4 py-2.5 sm:py-3">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-3">
          ...
          >
            <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-lg bg-rose-600/20 flex items-center justify-center overflow-hidden flex-shrink-0">
              {thumbnailUrl ? (
                <img src={thumbnailUrl} alt="" className="w-full h-full object-cover" />
              ) : (
                <FaMusic className="text-rose-400 text-sm" />
              )}
            </div>
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <button
              onClick={togglePlay}
              className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-rose-600 hover:bg-rose-700 flex items-center justify-center transition-all active:scale-90"
            >
